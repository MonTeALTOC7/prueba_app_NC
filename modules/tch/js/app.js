...
function renderHome() {
  ...
  return `
    ...
    <section class="kpi-grid">
      ...
      <article class="kpi gold"><span>TCH proyectado</span><strong>${latest.length?formatNumber(weighted, 1):"—"}</strong><small>${latest.length?"ponderado por área evaluada":"Sin biometrías guardadas"}</small></article>
      <article class="kpi red"><span>Toneladas</span><strong>${latest.length?formatNumber(tons, 0):"—"}</strong><small>${formatNumber(area, 1)} ha evaluadas</small></article>
    </section>
...
function renderLotEvolution(lot,records){
 const points=records.filter(r=>r.projectedTch!=null&&Number.isFinite(Number(r.projectedTch))).slice().sort((a,b)=>a.date.localeCompare(b.date));
 if(!points.length)return '';
 const max=Math.max(1,...points.map(r=>Number(r.projectedTch)));
 return `<section class="card"><div class="card-head"><strong>Proyección biométrica por fecha · ${escapeHtml(lot.id)}</strong></div><div class="card-body casur-tch-series">${points.map(r=>`<div class="casur-tch-series-row"><span>${escapeHtml(formatDateShort(r.date))}</span><div class="casur-tch-track" aria-hidden="true"><i style="width:${Math.max(0,Number(r.projectedTch))/max*100}%"></i></div><strong>${formatNumber(r.projectedTch,1)} TCH</strong></div>`).join('')}<p class="info-note">Hasta 20 evaluaciones recientes de esta suerte. Proyecciones registradas en cada fecha; no equivalen a TCH real cosechado.</p></div></section>`;
}
...
function renderAnalytics() {
  ...
  return `${pageHead("Consulta de suertes y registros", "Buscá el cronológico maestro, el TCH estimado Z26/27 y las evaluaciones realizadas.")}
    ...
    ${lot ? `${selectedLotPanel(lot, todayISO())}${renderLotEvolution(lot,biometries)}
...
// [INTEGRACIÓN App Maestra] Service Worker propio DESACTIVADO.
// Dentro de "Negocios de Caña CASUR" solo debe existir el SW raíz. Se
// neutraliza también el auto-reload por 'controllerchange' para que el
// iframe no se recargue cuando el SW maestro reclama el control (clients.claim).
// El repositorio fuente standalone conserva su SW intacto.
